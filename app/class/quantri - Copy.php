...
class quantri {
	...
	function __construct($url){
		...
		if ($action=="" || $action=="?td") $action="index";
		...
		if (method_exists($this,$action)==true) $this->$action();
		else echo "Nam mô A Di Đà Phật";
	}
	...
	function baiviet_loc(){
		$idloai=$this->params[0]; settype($idloai, "int");
		$tukhoa=$this->params[1]; 
		$dsbaiviet = $this->qt->baiviet_loc($idloai, $tukhoa);
	...
	function box_them_sexoa(){
		if ($_POST ==NULL) require_once "app/view/quantri/home.php";
		else{
			$kq = $this->qt->box_them();
			header('location:'. BASE_DIR.'quantri/box_list');
		}
	}
	function box_list(){
		require_once "app/view/quantri/home.php";
	...
	function box_sua_sexoa(){
		$id= $this->params[0]; settype($id,"int");
		if ($_POST ==NULL) {			
			$_SESSION['backbox']=$_SERVER['HTTP_REFERER'];
			$row = $this->qt->chitietbox($id);
			require_once "app/view/quantri/home.php";
		} else{			
			$kq = $this->qt->box_sua($id);
			if (isset($_SESSION['backbox'])==true){
				$backbox= $_SESSION['backbox'];
				unset($_SESSION['backbox']);
			}else $backbox= BASE_DIR. "quantri/box_list";			
			header('location:'. $backbox);
		}
	}
